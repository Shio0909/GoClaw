...
type Manager struct {
	provider  Provider
	rootProv  Provider // 非 nil 时从 rootProv 读 soul（scoped 模式）
	llmCall   LLMCaller
	turnCount int
...
// NewManager 创建记忆管理器
func NewManager(provider Provider, refineAt int) *Manager {
	if refineAt <= 0 {
		refineAt = 10
	}
	return &Manager{
...
// rootProv 用于读取 soul.md（全局人格），userProv 用于读写 user.md / memory.md / logs
func NewScopedManager(rootProv, userProv Provider, refineAt int) *Manager {
	if refineAt <= 0 {
		refineAt = 10
	}
	return &Manager{
...
// Store 返回底层 Provider
func (m *Manager) Store() Provider {
	return m.provider
}

// BuildContext 构建记忆上下文（注入到 system prompt）
func (m *Manager) BuildContext() (string, error) {
	// scoped 模式：soul 从 rootProv 读，user/memory 从 provider（userProv）读
	soulProv := m.provider
	if m.rootProv != nil {
		soulProv = m.rootProv
	}
	soul, err := soulProv.ReadSoul()
	if err != nil {
		return "", fmt.Errorf("read soul: %w", err)
	}
	user, err := m.provider.ReadUser()
	if err != nil {
		return "", fmt.Errorf("read user: %w", err)
	}
	mem, err := m.provider.ReadMemory()
	if err != nil {
...
func (m *Manager) OnTurn(ctx context.Context, role, content string) {
	_ = m.provider.AppendLog(LogEntry{
...
func (m *Manager) refine(ctx context.Context) {
	logs, err := m.provider.ReadTodayLogs()
	...
	currentMem, _ := m.provider.ReadMemory()
	currentUser, _ := m.provider.ReadUser()
...
(更新后的 user.md 完整内容)`, logText.String(), currentMem, currentUser)
	...
	if memStart := strings.Index(result, "---MEMORY---"); memStart != -1 {
		if userStart := strings.Index(result, "---USER---"); userStart != -1 {
			...
			if memContent != "" {
				_ = m.provider.WriteMemory(memContent)
			}
			if userContent != "" {
				_ = m.provider.WriteUser(userContent)
			}
		}
	}
}
